...
Elm.CounterStore = Elm.CounterStore || {};
Elm.CounterStore.make = function (_elm) {
   "use strict";
   _elm.CounterStore = _elm.CounterStore || {};
   if (_elm.CounterStore.values) return _elm.CounterStore.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var dispatchDecrement = Elm.Native.Port.make(_elm).inboundSignal("dispatchDecrement",
   "()",
   function (v) {
      return typeof v === "object" && v instanceof Array ? {ctor: "_Tuple0"} : _U.badPort("an array",v);
   });
   var dispatchIncrement = Elm.Native.Port.make(_elm).inboundSignal("dispatchIncrement",
   "()",
   function (v) {
      return typeof v === "object" && v instanceof Array ? {ctor: "_Tuple0"} : _U.badPort("an array",v);
   });
   var model0 = {value: 0};
   var update = F2(function (action,counter) {
      var _p0 = action;
      if (_p0.ctor === "Increment") {
            return _U.update(counter,{value: counter.value + 1});
         } else {
            return _U.update(counter,{value: counter.value - 1});
         }
   });
   ...
   var actions = $Signal.mergeMany(_U.list([A2($Signal.map,$Basics.always(Increment),dispatchIncrement)
                                           ,A2($Signal.map,$Basics.always(Decrement),dispatchDecrement)]));
   var modelChanges = A3($Signal.foldp,update,model0,actions);
   var counterChanges = Elm.Native.Port.make(_elm).outboundSignal("counterChanges",
   function (v) {
      return v;
   },
   A2($Signal.map,function (_) {    return _.value;},modelChanges));
   var Counter = function (a) {    return {value: a};};
   return _elm.CounterStore.values = {_op: _op
                                     ,Counter: Counter
                                     ,Increment: Increment
                                     ,Decrement: Decrement
                                     ,update: update
